...
namespace excel {
    ...
    int config_set_FaMapInfo::load_all() {
        return load_file("Maps/DefaultMap.bytes");
    }
    ...
    int config_set_FaMapInfo::load_file(const char* file_path, size_t sz) {
        ...
        if (!config_manager::me()->load_file_data(content, file_path_s)) {
            WLOGERROR("load file %s for %s failed", file_path_s.c_str(), "config_set_FaMapInfo");
            return -1;
        }
        ...
        if (!outer_data.ParseFromString(content)) {
            WLOGERROR("parse file %s for %s(message type: %s) failed, msg: %s",
                file_path_s.c_str(), "config_set_FaMapInfo", "FaGame::FaMapInfo_Tbl",
        ...
        for (; line_start < content.c_str() + content.size() && *line_start; line_start = line_end + 1) {
            ...
            if (res < 0) {
                WLOGERROR("load file %s from file list %s for %s failed, res: %d",
                    file_path.first, file_list_path, "config_set_FaMapInfo", res
                );
                ret = res;
            } else if (ret >= 0) {
                ret += res;
    ...
    void  config_set_FaMapInfo::merge_data(std::std::shared_ptr<item_type>) {
        // index: Id
    }
}