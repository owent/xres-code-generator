...
namespace FaGame {
    ...
    config_set_FaMapInfo::config_set_FaMapInfo() {
    }

    config_set_FaMapInfo::~config_set_FaMapInfo(){
    }

    int config_set_FaMapInfo::on_inited() {
    ...
    int config_set_FaMapInfo::load_all() {
        int ret = 0;
        ::util::lock::write_lock_holder wlh(load_file_lock_);
        for (std::unordered_map<std::string, bool>::iterator iter = file_status_.begin(); iter != file_status_.end(); ++ iter) {
            if (!iter->second) {
                ...
                if (res < 0) {
                    WLOGERROR("load config file %s for %s failed", iter->first.c_str(), "config_set_FaMapInfo");
                    ret = res;
                } else if (ret >= 0) {
                    ret += res;
                }
    ...
    void config_set_FaMapInfo::clear() {
        ::util::lock::write_lock_holder wlh(load_file_lock_);
        id_data_.clear();
        file_status_.clear();
        reload_file_lists();
    ...
    int config_set_FaMapInfo::load_file(const std::string& file_path) {
        std::unordered_map<std::string, bool>::iterator iter = file_status_.find(file_path);
        if (iter == file_status_.end()) {
            WLOGERROR("load config file %s for %s failed, not exist in any file_list/file_path", file_path.c_str(), "config_set_FaMapInfo");
            return -2;
        }
        ...
        if (!config_manager::me()->load_file_data(content, file_path)) {
            WLOGERROR("load file %s for %s failed", file_path.c_str(), "config_set_FaMapInfo");
            return -3;
        }

        ::FaGame::FaMapInfo_Tbl outer_data;
        if (!outer_data.ParseFromString(content)) {
            WLOGERROR("parse file %s for %s(message type: %s) failed, msg: %s",
                file_path.c_str(), "config_set_FaMapInfo", "::FaGame::FaMapInfo_Tbl",
    ...
    int config_set_FaMapInfo::load_list(const char* file_list_path) {
        std::string content;
        if (!config_manager::me()->load_file_data(content, file_list_path)) {
            WLOGERROR("load file %s for %s failed", file_list_path, "config_set_FaMapInfo");
            return -1;
        }
    ...
    int config_set_FaMapInfo::reload_file_lists() {
        file_status_["Maps/DefaultMap.bytes"] = false;
        return 0;
    }

    void config_set_FaMapInfo::merge_data(item_ptr_type item) {
        if (!item) {
            WLOGERROR("merge_data(nullptr) is not allowed for %s", "config_set_FaMapInfo");
            return;
        }
        ...
        do {
            ...
            if (id_data_.end() != id_data_.find(key)) {
                WLOGERROR("merge_data() with key=<%d> for %s is already exists, we will cover it with the newer value", 
                    static_cast<int>(item->id()), "config_set_FaMapInfo");
            }
            id_data_[key] = item;
        } while(false);
    ...
    config_set_FaMapInfo::id_value_type config_set_FaMapInfo::get_by_id(int32_t Id) {
        ...
        std::string file_path = "Maps/DefaultMap.bytes";

        int res = load_file(file_path);
        if (res < 0) {
            WLOGERROR("load file %s for %s failed, res: %d", file_path.c_str(), "config_set_FaMapInfo", res);
            return nullptr;
        }
        ...
        if (iter == id_data_.end()) {
            WLOGERROR("load index %s with key=<%d> for %s failed, not found",
                "id", static_cast<int>(Id), "config_set_FaMapInfo"
            );
            return nullptr;
        }
