...
%>      -- require("DataTableService").Get("${loader.code.class_name}"):GetByIndex("${code_index.name}", ${code_index.get_key_names()})
      { indexName = "${code_index.name}"${loader_data_source}, fullName = '${loader.full_name}', messageName = '${loader.pb_msg.name}', luaPath = '${loader.get_upb_lua_path()}', options = { isList = ${code_index_is_list}, allowNotFound = ${code_index_allow_not_found}, sortBy = { ${code_index.get_sort_by_names()} } }, keys = { ${code_index.get_key_names()} } },
%     endfor
  },
%   endfor
% endfor
