...
%   endfor
% endif
% for enum_full_path in dict(sorted(pb_file.pb_enums.items())):
<%
...
%   if not enum_value_has_zero:
    ${ue_excel_utils.UECppUEnumValueName(enum_inst, None, ue_bp_uenum_type_prefix)}__UNKNOWN = 0,
%   endif
%   for pb_enum_value_proto in enum_inst.descriptor_proto.value:
%     if enum_class_support_blue_print:
    ${ue_excel_utils.UECppUEnumValueName(enum_inst, pb_enum_value_proto, ue_bp_uenum_type_prefix)} = ${pb_enum_value_proto.number} UMETA(DisplayName="${pb_enum_value_proto.name}"),
%     else:
    ${ue_excel_utils.UECppUEnumValueName(enum_inst, pb_enum_value_proto, ue_bp_uenum_type_prefix)} = ${pb_enum_value_proto.number}, // ${pb_enum_value_proto.name}
%     endif
%   endfor
};
% endfor
...
%       endif
{
    ${ue_excel_utils.UECppUOneofEnumValueName(oneof_inst, None, ue_bp_uenum_type_prefix)} = 0,
%       for pb_field_name in oneof_inst.fields:
<%
pb_field_inst = oneof_inst.fields[pb_field_name]
current_enum_field_name = ue_excel_utils.UECppUOneofEnumValueName(oneof_inst, pb_field_inst, ue_bp_uenum_type_prefix)
%>
%         if oneof_class_support_blue_print:
