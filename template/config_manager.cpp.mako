...
EXCEL_CONFIG_LOADER_API int config_manager::init_new_group() {
  std::string version;
  {
    ...
    if (enable_multithread_lock_) {
      rlh = ::excel::lock::read_lock_holder<::excel::lock::spin_rw_lock>{handle_lock_};
    }
  ...
  do {
    // 检查版本号
    ::excel::lock::read_lock_holder<::excel::lock::spin_rw_lock> wlh;
    if (enable_multithread_lock_) {
      wlh = ::excel::lock::read_lock_holder<::excel::lock::spin_rw_lock>{config_group_lock_};
    }
...
EXCEL_CONFIG_LOADER_API void config_manager::reset() {
  std::unordered_map<void *, std::function<void()>> on_evt_reset;
  {
    excel::lock::write_lock_holder<::excel::lock::spin_rw_lock> wlh;
    if (enable_multithread_lock_) {
      wlh = excel::lock::write_lock_holder<::excel::lock::spin_rw_lock>{evt_lock_};
    }
...

  {
    ::excel::lock::write_lock_holder<::excel::lock::spin_rw_lock> wlh;
    if (enable_multithread_lock_) {
      wlh = ::excel::lock::write_lock_holder<::excel::lock::spin_rw_lock>{handle_lock_};
    }
    max_group_number_ = 8;
    override_same_version_ = false;
...
EXCEL_CONFIG_LOADER_API const config_manager::config_group_ptr_t& config_manager::get_current_config_group() {
...

  {
    excel::lock::read_lock_holder<::excel::lock::spin_rw_lock> rlh;
    if (enable_multithread_lock_) {
      rlh = excel::lock::read_lock_holder<::excel::lock::spin_rw_lock>{config_group_lock_};
    }
    if (!config_group_list_.empty()) {
